...
contract ProposalMasterCharlieV1 is IProposalMaster, Ownable {
    ...
    function updateProposalPercentage(uint256 percentage) public onlyOwner {
        require(
            percentage <= 100,
            "SimpleProposalMaster::setPercentage: Invalid percentage"
        );

        require(
            percentage != proposalPercentage,
            "SimpleProposalMaster::setPercentage: Value is already in use"
        );

        emit ProposalPercentageUpdated(proposalPercentage, percentage);
